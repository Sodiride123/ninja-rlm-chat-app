...
def find_final_answer(text: str, environment: "BaseEnv | None" = None) -> str | None:
    ...
    # Use greedy match and find the last closing paren that balances
    final_pattern = r"^\s*FINAL\((.+)\)"
    match = re.search(final_pattern, text, re.MULTILINE | re.DOTALL)
    if match:
        content = match.group(1)
        # Handle nested parentheses by finding the balanced closing paren
        # Count from the end to find where the FINAL() actually ends
        depth = 0
        for i, char in enumerate(content):
            if char == '(':
                ...
                if depth == 0:
                    # This is the matching closing paren for FINAL(
                    return content[:i].strip()
                depth -= 1
        # No nested parens issue, return full content
