...
class RLM:
    ...
    def _completion_turn(
        ...
        Emits progress events incrementally via verbose printer as each step completes.
        """
        iter_start = time.perf_counter()
        response = lm_handler.completion(prompt)
        llm_time = time.perf_counter() - iter_start

        # Emit LLM response event immediately after receiving it
        ...
        for code_block_str in code_block_strs:
            ...
            code_block = CodeBlock(code=code_block_str, result=code_result)
            code_blocks.append(code_block)

            # Emit code execution event immediately after each block completes
            self.verbose.print_code_execution(code_block)

            # Emit subcall events for any sub-LLM calls made during code execution
            for call in code_result.rlm_calls:
                self.verbose.print_subcall(
                    model=call.root_model,
                    prompt_preview=str(call.prompt)[:200] if call.prompt else "",
                    response_preview=str(call.response)[:500] if call.response else "",
